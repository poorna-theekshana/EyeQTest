...
class HiddenWordActivity2 : AppCompatActivity(), View.OnTouchListener {
    ...
    private fun showGeneratedWordsDialog() {
        ...
        for (word in words) {
            for (i in 0 until gridSize) {
                for (j in 0 until gridSize) {
                    if (word2Arrays[words.indexOf(word)].checkLoc(i * 10 + j, i * 10 + j + word.length - 1, true) ||
                        word2Arrays[words.indexOf(word)].checkLoc(i * 10 + j, i * 10 + j + (word.length - 1) * 10, false)
                    ) {
                        generatedWords.add(word)
                        if (word2Arrays[words.indexOf(word)].found) {
                            foundWords.add(word)
                        }
                    }
                }
        ...
        if (generatedWords.isNotEmpty()) {
            ...
            for (foundWord in foundWords) {
                ...
                val colorSpan = ForegroundColorSpan(ContextCompat.getColor(this, R.color.bblue)) // Replace with your green color resource
    ...
    private fun checkIfRangeIsValid(initTag: String, endTag: String){
        ...
        val dialogView = inflater.inflate(R.layout.congrats_layout2, null)
        ...
        dialogView.findViewById<Button>(R.id.okButton).setOnClickListener {
            congratsDialog.dismiss()
            val mainActivityIntent = Intent(this@HiddenWordActivity2,MainActivity::class.java)
                ...
                for (n in 0 until gridSize){
                    ...
                    if(found) {
                        ...
                        if(toggle){
                            ...
                            word2Arrays[wordIndex].setLoc(_n*10 + r, toggle)
                        } else {
                            // Use the index of the word in the original 'words' array
                            val wordIndex = words.indexOf(word)
                            word2Arrays[wordIndex].setLoc(r*10 + _n, toggle)
                        }
    ...
    companion object {
        ...
        val word2Arrays = Array<Word2>(numWords) { Word2("")}
